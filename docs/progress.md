...
## 2026-02-06（Build #19 - 键盘修复 + Exit 修复）

### 已完成

修复了两个关键问题：

#### 1. 键盘输入 ERROR_INVALID_PARAMETER (87)
- **问题**：`/input/key` 调用返回 `lastError=87`，键盘输入完全不工作
- **根因**：`KeyInput.cs` 中的 `InputUnion` 结构体只包含 `KEYBDINPUT`，导致 `Marshal.SizeOf<INPUT>()` 返回错误的大小（x64 上应为 40 字节，实际只有 32 字节）
- **修复**：在 `InputUnion` 中添加 `MOUSEINPUT` 确保正确的 union 大小

#### 2. Exit 无法退出
- **问题**：点击托盘菜单的 "Exit" 后应用无响应，无法退出
- **根因**：`ApiHost.Dispose()` 在 WPF UI 线程上调用 `StopAsync().GetAwaiter().GetResult()` 导致死锁
- **修复**：使用 `Task.Run` 包装异步调用，添加 3 秒超时
